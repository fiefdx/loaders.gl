...
import {DracoLoader, DracoWriter, DracoEncoder} from '@loaders.gl/draco';
...
test('DracoEncoder#encode(bunny.drc)', async t => {
  ...
  for (const tc of TEST_CASES) {
    ...
    t.doesNotThrow(() => {
      ...
      compressedMesh = tc.options.pointcloud
        ? dracoEncoder.encodePointCloud(attributes)
        : dracoEncoder.encodeMesh(attributes);
...
test('DracoWriter#encodeFileSync(bunny.drc)', async t => {
  ...
  for (const tc of TEST_CASES) {
    ...
    let compressedMesh;
    t.doesNotThrow(() => {
      compressedMesh = encodeFileSync(attributes, DracoWriter, tc.options);
      const ratio = meshSize / compressedMesh.byteLength;
      t.comment(`${tc.title} ${compressedMesh.byteLength} bytes, ratio ${ratio.toFixed(1)}`);
    }, `${tc.title} did not trow`);
